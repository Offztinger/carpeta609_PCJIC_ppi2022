...
const TeamProvider = ({ children }) => {
	...
	const url = 'http://3.147.42.128:4000/teams';
	const getTeams = async () => {
		await GETRequest(`${url}/front`, setTeams);
	};

	const getTeamMembers = async id => {
		await GETRequest(`http://3.147.42.128:4000/teamMember/${id}`, setTeamMembers);
	};
	...
	const deleteTeam = async id => {
		await axios
			.get(`http://3.147.42.128:4000/logbook/${id}`, {
				headers: {
			...
			.then(async res => {
				if (res.data?.id != undefined) {
					await DELETERequest(`http://3.147.42.128:4000/logbook`, res.data.id);
				}
